...
AS
BEGIN
	SELECT * FROM EMPLOYEE WHERE FIRSTNAME = @FNAME OR LASTNAME = @LNAME
END;

EXEC PR_GET_EMPLOYEE_DETAILS 'JOHN' , 'DOE';
--2. Create a Procedure that will accept Department Name and based on that gives employees list who
--belongs to that department. CREATE OR ALTER PROCEDURE PR_GET_EMPLOYEE_DETAILS_FROMDEPTNAME
@DEPNAME VARCHAR(50)
AS
BEGIN
	SELECT * FROM EMPLOYEE JOIN Departments ON Employee.DepartmentID = Departments.DepartmentID WHERE Departments.DepartmentName = @DEPNAME;
END;

EXEC PR_GET_EMPLOYEE_DETAILS_FROMDEPTNAME 'HR';
...
CREATE OR ALTER PROCEDURE PR_GET_PROJECTDETAILS
@PRONAME VARCHAR(50),
@DEPNAME VARCHAR(50)
AS
BEGIN
	SELECT * FROM Projects JOIN Departments ON Projects.DepartmentID = Departments.DepartmentID WHERE Departments.DepartmentName = @DEPNAME AND Projects.ProjectName = @PRONAME;
END;
...
CREATE OR ALTER PROCEDURE PR_GET_EMPDETAILSFROMSAL
@SAL1 INT,
@SAL2 INT
AS
BEGIN
...
CREATE OR ALTER PROCEDURE PR_GET_EMPDETAILSFROMHDATE
@HDATE DATETIME
AS
BEGIN
...
--served.
CREATE OR ALTER PROCEDURE PR_GET_EMPDETAILSFROMHGEND
@GENDER VARCHAR(10)
AS
BEGIN
...
EXEC PR_GET_EMPDETAILSFROMHGEND 'M';
...
CREATE OR ALTER PROCEDURE PR_GET_DEPDETAILSFROMLOC
@LOC VARCHAR(10)
AS
BEGIN
	SELECT * FROM Employee WHERE Gender LIKE @LOC + '%';
END;

--Part – C
--9. Create a procedure that will accepts From Date & To Date and based on that he/she will retrieve Project
--related data.
CREATE OR ALTER PROCEDURE PR_GET_PRODETAILSFROMDATE
@FDATE DATETIME,
@LDATE DATETIME
AS
BEGIN
	SELECT * FROM Projects WHERE StartDate = @FDATE AND EndDate = @LDATE;
END;
...
CREATE OR ALTER PROCEDURE PR_GET_
@PRONAME VARCHAR(50),
@LOC VARCHAR(50)
AS
BEGIN
	SELECT * FROM PROJECTS JOIN Departments ON Projects.DepartmentID = Departments.DepartmentID WHERE Projects.ProjectName = @PRONAME AND Departments.Location = @LOC;
END;
